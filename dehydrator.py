...
CANONICAL_DOMAINS = ("关系", "项目", "生活", "创作", "内心", "未分类")
DOMAIN_ALIASES = {
    "人际": "关系", "家庭": "关系", "恋爱": "关系", "友谊": "关系", "社交": "关系",
    "工作": "项目", "学习": "项目", "考试": "项目", "求职": "项目", "编程": "项目",
    "AI": "项目", "ai": "项目", "硬件": "项目", "网络": "项目", "数字": "项目",
    "事务": "项目", "计划": "项目", "待办": "项目", "财务": "项目",
    "日常": "生活", "生活": "生活", "饮食": "生活", "穿搭": "生活", "出行": "生活",
    "居家": "生活", "购物": "生活", "健康": "生活", "睡眠": "生活", "运动": "生活",
    "兴趣": "创作", "游戏": "创作", "影视": "创作", "音乐": "创作", "阅读": "创作",
    "手工": "创作",
    "情绪": "内心", "心理": "内心", "回忆": "内心", "梦境": "内心", "自省": "内心",
    "成长": "内心",
}
DIARY_MARKERS = {"diary", "日记", "日记本"}


def normalize_memory_meta(domain, tags) -> tuple[list[str], list[str]]:
    """Clamp LLM generated domains to the small canonical set."""
    raw_domains = domain if isinstance(domain, list) else [domain]
    ...
    for item in raw_domains:
        ...
        if not value:
            continue
        if value in DIARY_MARKERS or value.lower() == "diary":
            if "日记" not in clean_tags:
                clean_tags.append("日记")
            if "diary" not in clean_tags:
                clean_tags.append("diary")
            continue
        mapped = value if value in CANONICAL_DOMAINS else DOMAIN_ALIASES.get(value)
        if mapped and mapped not in mapped_domains:
            mapped_domains.append(mapped)

    if not mapped_domains:
        mapped_domains = ["未分类"]
    return mapped_domains[:2], clean_tags[:15]
...
主题域只能从以下 6 个大类里选 1 个，最多 2 个：关系、项目、生活、创作、内心、未分类。
不要创建新的主题域；日记不是主题域，日记内容请在 tags 里加“日记”和“diary”。
...
分析规则：
1. domain（主题域）：只能从以下 6 个大类里选 1 个，最多 2 个：关系、项目、生活、创作、内心、未分类。不要创建新的主题域；日记不是主题域，日记内容请在 tags 里加“日记”和“diary”。
...
class Dehydrator:
    ...
    def _parse_analysis(self, raw: str) -> dict:
        ...
        domain, tags = normalize_memory_meta(result.get("domain", ["未分类"]), result.get("tags", []))
        return {
            "domain": domain,
            "valence": valence,
            "arousal": arousal,
    ...
    def _parse_digest(self, raw: str) -> list[dict]:
        ...
        for item in items:
            ...
            domain, tags = normalize_memory_meta(item.get("domain", ["未分类"]), item.get("tags", []))
            validated.append({
