...
async def breath(
    ...
    # --- Feel retrieval: domain="feel" is a special channel (before no-query surfacing) ---
    # --- Feel 检索：domain="feel" 是独立入口，在浮现模式之前拦截 ---
...
async def api_diary(request):
    """Public endpoint: return diary buckets (domain contains 'diary'). No auth required."""
    from starlette.responses import JSONResponse
    try:
        all_buckets = await bucket_mgr.list_all(include_archive=False)
        result = []
        for b in all_buckets:
            ...
            if is_diary:
                result.append({
                    ...
                    "name": meta.get("name", b["id"]),
                    "created": meta.get("created", ""),
                    "content_preview": strip_wikilinks(b.get("content", ""))[:2000],
                })
...
async def diary_page(request):
    """Serve the diary HTML page."""
    from starlette.responses import HTMLResponse
    import os
    diary_path = os.path.join(os.path.dirname(__file__), "diary.html")
    try:
        with open(diary_path, "r", encoding="utf-8") as f:
            return HTMLResponse(f.read())
    except FileNotFoundError:
        return HTMLResponse("<h1>diary.html not found</h1>", status_code=404)
